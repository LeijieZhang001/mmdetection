...
// array_wrapper.hpp is needed if using boost version >= 1.64
...
enum METRIC{IMAGE=0, GROUND_IOU5=1, GROUND_IOU7=2, BOX3D_IOU25=3, BOX3D_IOU5=4, BOX3D_IOU7=5};
...
// point type for computing distance error
enum POINT_TYPE{CENTER=0, CLOSEST=1};

// parameters varying per class
vector<string> CLASS_NAMES;
// the minimum overlap required for 2D evaluation on the image/ground plane and 3D evaluation
const double MIN_OVERLAP[6][3] = {{0.7, 0.5, 0.5}, {0.5, 0.25, 0.25}, {0.7, 0.5, 0.5}, {0.25, 0.25, 0.25}, {0.5, 0.5, 0.5}, {0.7, 0.7, 0.7}};
...
template <typename T>
std::vector<double> boxPoint(const T& g, POINT_TYPE point_type) {
    using namespace boost::numeric::ublas;
    using namespace boost::geometry;
    matrix<double> mref(2, 2);
    mref(0, 0) = cos(g.ry); mref(0, 1) = sin(g.ry);
    ...
    if (point_type == CENTER) {
      point.push_back((gc(0,0) + gc(0,1) + gc(0,2) + gc(0,3)) / 4.0);
      point.push_back((gc(1,0) + gc(1,1) + gc(1,2) + gc(1,3)) / 4.0);
    } else if (point_type == CLOSEST) {
      double min_dist = 1000000;
      int min_id = 0;
      for (int i = 0; i < 4; ++i) {
        double dist = sqrt(gc(0,i) * gc(0,i) + gc(1,i) * gc(1,i));
        if (dist < min_dist) {
          min_id = i;
          min_dist = dist;
        }
      }
      point.push_back(gc(0, min_id));
      point.push_back(gc(1, min_id));
    }
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  for(int32_t i=0;i<det.size(); i++){
    ...
    if(height<MIN_HEIGHT[difficulty] && det[i].box.y1 >= 0 && det[i].box.y2 >= 0)
...
void computeMeanDist(CLASSES current_class, const vector<tGroundtruth> &gt,
        ...
        METRIC metric, vector<double> &dist, vector<int> &count,
        POINT_TYPE point_type, double dist_step=5.0, double dist_range=100.0, double thresh=0){
  ...
  int N_dist = ceil(dist_range / dist_step);
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    // found a valid true positive
      ...
      /*========================================================================
      compute mean error of location
      ========================================================================*/
      vector<double> dt_pt = boxPoint(det[det_idx], point_type);
      vector<double> gt_pt = boxPoint(gt[i], point_type);
      double dx = dt_pt[0] - gt_pt[0];
      double dz = dt_pt[1] - gt_pt[1];
      double dist_err = sqrt(dx * dx + dz * dz);
      double gt_dist = sqrt(gt_pt[0] * gt_pt[0] + gt_pt[1] * gt_pt[1]);
      // count by ranges
      for (int k = 0; k < N_dist; k++) {
          if (gt_dist >= k * dist_step && gt_dist < (k + 1) * dist_step) {
              dist[k] += dist_err / gt_dist;
              count[k] +=1;
              break;
          }
      }
...
bool eval_dist (CLASSES current_class,
        ...
        DIFFICULTY difficulty, METRIC metric, double &m_dist,
  ...
  double dist_step = 5.0;
  double dist_range = 100.0;
  int N_dist = ceil(dist_range / dist_step);
  vector<double> dist(N_dist, 0.0);
  vector<int> count(N_dist, 0);
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    // compute statistics to get distance error
    computeMeanDist(current_class, groundtruth[i], detections[i], dc, i_gt, i_det, false, boxoverlap, metric, dist, count, point_type, dist_step, dist_range);
  }
...
vector<int32_t> getEvalIndices(const string& result_dir) {
    ...
    if (dir) {
        while (entity = readdir(dir)) {
            ...
            if (len < 10) continue;
            int32_t index = atoi(path.substr(len - 10, 6).c_str());
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  // eval mean distance error of box center based on image box IoU
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c] && eval_image[c]) {
      double m_dist;
      if(!eval_dist(cls, groundtruth, detections, imageBoxOverlap, HARD, IMAGE, m_dist, CENTER)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      printf("%s mean center distance ratio (IM IoU = 0.7): %f\n", CLASS_NAMES[c].c_str(), m_dist);
    }
  }

  // eval mean distance error of box center based on ground box IoU
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      double m_dist;
      if(!eval_dist(cls, groundtruth, detections, groundBoxOverlap, HARD, GROUND_IOU5, m_dist, CENTER)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      printf("%s mean center distance ratio (BV IoU = 0.5): %f\n", CLASS_NAMES[c].c_str(), m_dist);
    }
  }

  // eval mean distance error of closest points based on image box IoU
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c] && eval_image[c]) {
      double m_dist;
      if(!eval_dist(cls, groundtruth, detections, imageBoxOverlap, HARD, IMAGE, m_dist, CLOSEST)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      printf("%s distance to impact ratio (IM IoU = 0.7): %f\n", CLASS_NAMES[c].c_str(), m_dist);
    }
  }

  // eval mean distance error of closest points based on ground box IoU
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      double m_dist;
      if(!eval_dist(cls, groundtruth, detections, groundBoxOverlap, HARD, GROUND_IOU5, m_dist, CLOSEST)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      printf("%s distance to impact ratio (BV IoU = 0.5): %f\n", CLASS_NAMES[c].c_str(), m_dist);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground_iou5.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[0], aos[0], EASY, GROUND_IOU5)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[1], aos[1], MODERATE, GROUND_IOU5)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[2], aos[2], HARD, GROUND_IOU5)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_detection_ground_iou5", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground_iou7.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[0], aos[0], EASY, GROUND_IOU7)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[1], aos[1], MODERATE, GROUND_IOU7)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, groundBoxOverlap, precision[2], aos[2], HARD, GROUND_IOU7)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_detection_ground_iou7", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_3d_iou25.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[0], aos[0], EASY, BOX3D_IOU25)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[1], aos[1], MODERATE, BOX3D_IOU25)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[2], aos[2], HARD, BOX3D_IOU25)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_detection_3d_iou25", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_3d_iou5.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[0], aos[0], EASY, BOX3D_IOU5)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[1], aos[1], MODERATE, BOX3D_IOU5)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[2], aos[2], HARD, BOX3D_IOU5)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_detection_3d_iou5", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_3d_iou7.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[0], aos[0], EASY, BOX3D_IOU7)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[1], aos[1], MODERATE, BOX3D_IOU7)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[2], aos[2], HARD, BOX3D_IOU7)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_detection_3d_iou7", CLASS_NAMES[c], precision, 0);
    }
  }
